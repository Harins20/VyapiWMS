...
const Shipto = () => {
    ...
    const [permissions, setpermissions] = useState(shipto)
    ...
    useEffect(() => {
        setLoading(true)
        loadingGridValues();
        getRoles(token, "shipto").then((res) => {
            setpermissions(res)
        })
    }, [token])
    ...
    const editclick = e => {
        navigate('/Config/Shipto/Creation', { state: { Shiptodetail: e } })
    }
    ...
    const deleteclick = () => {
        ...
        var timeout;
        deletetorerKeys(finalobj, token, 2).then(res => {
                                ...
                                <ul className='header-buttons'>
                                    <ButtonGroup>
                                        <Button  onClick={() => {
                                            setLoading(true)
                                            loadingGridValues();
                                        }} ><span class="k-icon k-i-reload-sm" style={{ fontSize: "20px", cursor: "pointer", marginRight: "5px" }}></span> Refresh</Button>
                                        {permissions.create.visible === 'True' && <Button className='text-uppercase br-round active' type="button" onClick={() => {
                                            navigate('/Config/Shipto/Creation')
                                        }}  >New</Button>}
                                       {permissions.delete.visible === 'True' && <Button className='text-uppercase br-round-right active' type='button' onClick={toggleDialog} fillMode="solid" >Delete</Button>}
                                    </ButtonGroup>
                                </ul>
                            </div>
                        </GridToolbar>
                        {permissions.delete.visible === 'True' && <Column title="" filterable={false} className="edit-btn" width={60} cell={Deletecell} />}
                        <Column field="StorerKey" title="Ship To" width={250} filterable={true} />
