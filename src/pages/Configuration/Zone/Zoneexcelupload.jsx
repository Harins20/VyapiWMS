...
import { putZone } from "../../../services/ConfigurationService/ConfigurationService"
...
const Zoneexcelupload = () => {
    ...
    function fileChangehandle(e) {
        ...
        reader.onload = function (e) {
            ...
            debugger
            if (JSON.stringify(["Descr", "PutawayZone"]) === JSON.stringify(columnHeaders.sort())) {
                putZone(JSON.stringify(data), token, "1").then(res => {
                    var timeout;
                    setLoading(false);
                    if (res === "Zone stored") {
                        setopen(true);
                        setmessage("Zones Created Successfully")
                        setseverity("success")
                        ...
                        setopen(true);
                        setmessage("Zones not created successfully")
                        setseverity("error")
                        clearTimeout(timeout);
                        timeout = setTimeout(function () {
                            setopen(false);
            ...
            {
                setopen(true);
                setmessage("Excel must contain Descr, PutawayZone columns")
                setseverity("error")
                clearTimeout(timeout);
                timeout = setTimeout(function () {
                    setopen(false);
    ...
    return (<>
        ...
        <WithSnackbar open={open} message={message} severity={severity} />
            <div className="d-flex mt-5 justify-content-between">
                <div className='ps-3'>
                    <h3 className='fw-bold text-uppercase'>{Client}-{'Zone Excel Upload'}</h3>
                </div>
            </div>
        </div>
        <div className='col-3'>
...
export default Zoneexcelupload;